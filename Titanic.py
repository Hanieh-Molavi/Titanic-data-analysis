...
for c in range(0,len(recogonize_boolean)):
	...
	elif p==len(check_boolean) or p>z:
...
for m in range(0,len(mean_columns)):
	...
	plt.xlabel(mean_columns[m])
	plt.ylabel('Counter')
	plt.plot(counter,df[mean_columns[m]],color='darkviolet')
	plt.pause(4)
	name=str(m)+'LinearPlot'
	plt.savefig(name)
	plt.close()
	name=0
...
for n in range(0,len(float_columns_names)):
	...
	plt.xlabel(float_columns_names[n])
	plt.ylabel('Counter')
	plt.plot(counter,df[float_columns_names[n]],color='darkviolet')
	plt.pause(4)
	name=str(n)+'LinearPlot'
	plt.savefig(name)
	plt.close()
	name=0
...
plt.pause(4)
plt.savefig('PiePolt.png')
plt.close()



float_columns_names.remove('age')
...
#checkDistance
print("\n *________________________ Similority ___________________________*\n")
for m in range(0,len(mean_columns)):
	distance=[mean_columns[m]]
	da=pd.read_csv('Titanic.csv',usecols=distance)
	data=da.to_numpy()
	mid=len(data)/2
	mid=int(mid)
